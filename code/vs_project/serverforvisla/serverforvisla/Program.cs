...
class Server
{
    ...
    static void AuthorizationData(string prefix)
    {
        ...
        Console.Write(" done.\n" + prefix + "Adding usernames to tokens dictionary...");
        tokens.Add("login", null);
    }
    ...
    static byte[] HandleData(string data, string prefix = "", string secondprefix = "")
    {
        ...
        {
            case "files":
                return REQFiles(url.Substring(parts[0].Length + 1), prefix + secondprefix);
        }
    ...
    static bool HeaderAuth(Dictionary<string, string> headers, string prefix)
    {
        ...
        foreach (var cook in c)
        {
            ...
            Cookies.Add(kvp[0], cook.Substring(kvp[0].Length + 2).Trim(' ').Trim('\r'));
        }
